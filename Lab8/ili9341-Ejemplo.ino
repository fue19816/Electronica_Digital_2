...
//***************************************************************************************************************************************
void MapSD(char x[]){
  imagen = SD.open(x, FILE_READ);  //Open image
  int hex = 0;
  int v1 = 0;
  int v2 = 0;
  int mapar = 0;
  int vert = 0;
  if(imagen){  //If the image file opened
    Serial.println("Leyendo la imagen...");
    while(vert <= 239){
      mapar = 0;
      while (mapar<640){
        hex = imagen.read();
        if(hex==120){    //Check for the x of 0xFF hex
          v1 = imagen.read();  //Read the next 2 characters which will be the hex code
          v2 = imagen.read();
          v1 = ASCII_Hex(v1);
          v2 = ASCII_Hex(v2);
          maps[mapar] = (v1*16)+v2;
    ...
    Serial.println("No se pudo encontrar la imagen");
    imagen.close();
  }
}
//***************************************************************************************************************************************
